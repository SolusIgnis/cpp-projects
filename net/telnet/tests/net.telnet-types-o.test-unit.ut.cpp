...
suite net_telnet_types_byte_tests = [] {
    using net::telnet::byte_t;

    "byte_t is exactly one byte"_test = [] {
        expect(sizeof(byte_t) == 1_u);
    };

    "byte_t behaves as unsigned 8-bit storage"_test = [] {
        byte_t b = static_cast<byte_t>(255);
        expect(static_cast<unsigned>(b) == 255_u);
    };
};

suite net_telnet_types_command_value_tests = [] {
    using net::telnet::command;

    "IAC value matches RFC"_test = [] {
        expect(std::to_underlying(command::iac) == 0xFF_u);
    };

    "WILL value matches RFC"_test = [] {
        expect(std::to_underlying(command::will_opt) == 0xFB_u);
    };

    "WONT value matches RFC"_test = [] {
        expect(std::to_underlying(command::wont_opt) == 0xFC_u);
    };

    "DO value matches RFC"_test = [] {
        expect(std::to_underlying(command::do_opt) == 0xFD_u);
    };

    "DONT value matches RFC"_test = [] {
        expect(std::to_underlying(command::dont_opt) == 0xFE_u);
    };
};

suite net_telnet_types_command_format_tests = [] {
    using net::telnet::command;

    "default format prints name and hex"_test = [] {
    ...
    "unknown command name-only prints UNKNOWN"_test = [] {
        auto unknown = static_cast<command>(0x00);
        auto s       = std::format("{:n}", unknown);
        expect(s == "UNKNOWN");
    };

    "unknown command hex-only prints raw hex"_test = [] {
        auto unknown = static_cast<command>(0x01);
        auto s       = std::format("{:x}", unknown);
        expect(s == "0x01");
    };

    "invalid format specifier throws"_test = [] {
...
suite net_telnet_types_negotiation_direction_tests = [] {
    using net::telnet::negotiation_direction;

    "local formats correctly"_test = [] {
        auto s = std::format("{}", negotiation_direction::local);
        expect(s == "local");
    };

    "remote formats correctly"_test = [] {
