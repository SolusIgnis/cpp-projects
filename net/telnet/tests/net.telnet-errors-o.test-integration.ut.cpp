...
"make_error_code(error) integration"_test = [] mutable {
    ...
    expect(eq(ec.value(),
              static_cast<int>(error::invalid_command)));

    expect(eq(std::string{ec.category().name()},
              std::string{"telnet"}));
};
...
"make_error_code(processing_signal) integration"_test = [] mutable {
    ...
    expect(eq(ec.value(),
              static_cast<int>(processing_signal::go_ahead)));
