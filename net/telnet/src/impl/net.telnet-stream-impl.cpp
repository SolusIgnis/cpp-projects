...
namespace net::telnet {
    ...
    stream<NLS, PC>::stream(next_layer_type&& next_layer_stream)
        : next_layer_(std::move(next_layer_stream)), fsm_()
    {
        std::error_code ec;
    ...
    void stream<NLS, PC>::InputProcessor<MBS>::handle_processor_state_initializing(Self& self) {
        ...
        if (context_.input_side_buffer.size() == 0) {
            ...
            parent_stream_.next_layer().async_read_some(
                read_buffer,
    ...
    template<MutableBufferSequence MBS>
    template<typename Self>
    void stream<NLS, PC>::InputProcessor<MBS>::do_response(typename stream::fsm_type::NegotiationResponse response, Self&& self) {
        parent_stream_.async_write_negotiation(
            response,
    ...
    void stream<NLS, PC>::InputProcessor<MBS>::do_response(
        awaitables::SubnegotiationAwaitable awaitable, Self&& self) {
        asio::co_spawn(
    ...
    void stream<NLS, PC>::InputProcessor<MBS>::do_response(std::tuple<awaitables::TaggedAwaitable<Tag, T, Awaitable>, std::optional<typename stream::fsm_type::NegotiationResponse>> response, Self&& self) {
