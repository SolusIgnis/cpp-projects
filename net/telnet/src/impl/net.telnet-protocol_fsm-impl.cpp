...
namespace net::telnet {
     ...
     */
    template<typename PC>
    std::tuple<std::error_code, std::optional<typename ProtocolFSM<PC>::NegotiationResponse>>
        ProtocolFSM<PC>::request_option(option::id_num opt, negotiation_direction direction) {
        if (!ProtocolConfig::registered_options.get(opt)) {
            ProtocolConfig::log_error(make_error_code(error::option_not_available),
        ...
        if (status.enabled(direction)) { //YES
            ProtocolConfig::log_error(make_error_code(error::invalid_negotiation),
            ...
            return {
                std::error_code{},
                NegotiationResponse{direction, true, opt}
            };
        }
        ProtocolConfig::log_error(make_error_code(error::protocol_violation),
    ...
    std::tuple<std::error_code,
               std::optional<typename ProtocolFSM<PC>::NegotiationResponse>,
        ...
        if (status.disabled(direction)) { //NO
            ...
            return {
                std::error_code{},
                NegotiationResponse{direction, false, opt},
                std::move(awaitable)
            };
        }
        ProtocolConfig::log_error(make_error_code(error::protocol_violation),
            ...
            if (current_command_) [[likely]] {
                ...
                switch (*current_command_) {
                    ...
                    case se:
                        ...
                        ProtocolConfig::log_error(make_error_code(error::invalid_subnegotiation),
                                                  "byte: 0x{:02x}, cmd: {}, opt: {}",
                                                  byte,
                                                  telnet::command::se,
                                                  current_option_);
                    ...
                    case ga:
                        if (option_status_[option::id_num::suppress_go_ahead].enabled(negotiation_direction::remote)) {
                            ...
                            ProtocolConfig::log_error(make_error_code(error::ignored_go_ahead),
                    ...
                    case brk:
                        ...
                        break;
                    default:
                        ProtocolConfig::log_error(make_error_code(error::invalid_command),
                                                  "byte: 0x{:02x}, cmd: {}, opt: {}",
                                                  byte,
                                                  *current_command_,
                                                  current_option_);
    ...
    std::tuple<std::error_code, bool, std::optional<typename ProtocolFSM<PC>::ProcessingReturnVariant>>
        ...
        if (byte == std::to_underlying(telnet::command::iac)) {
            ...
            if (max_size > 0 && subnegotiation_buffer_.size() >= max_size) {
                ProtocolConfig::log_error(make_error_code(error::subnegotiation_overflow),
                                          "byte: 0x{:02x}, cmd: {}, opt: {}",
                                          byte,
                                          current_command_,
                                          *current_option_);
     ...
     */
    template<typename PC>
    awaitables::SubnegotiationAwaitable ProtocolFSM<PC>::handle_status_subnegotiation(const option opt,
                                                                                      std::vector<byte_t> buffer) {
        constexpr byte_t IS   = static_cast<byte_t>(0);
        constexpr byte_t SEND = static_cast<byte_t>(1);

        if (buffer.empty()) {
            ProtocolConfig::log_error(error::invalid_subnegotiation,
