...
## Purpose: This document compiles all `@todo` tasks from the Telnet project files, organized by phase, to guide development in Phase 6 and beyond. The tasks focus on enhancing socket compatibility, TLS support, and interface cleanup, building on the modular structure defined in `telnet.cppm`.

## Development Plan

### Overview
The Telnet project (version 0.5.0) has completed Phases 4 and 5, achieving protocol stability through RFC 1143 compliance, enhanced logging with `std::format`, urgent data/Synch procedure handling (RFC 854), advanced option handling with tagged awaitables, handler registration, and user-initiated negotiation. Phase 6 targets socket compatibility and interface cleanup, while Future Development addresses long-term maintenance and optimization. Phase 6 and later milestones are unordered, with changes requiring explicit approval.
...
- [ ] **Half-Duplex Support**:
  ...
  - **Steps**:
    ...
    - Update `ProtocolFSM`, `socket`, and `socket::InputProcessor` to handle half-duplex constraints (i.e., `TelnetCommand::GA` handling).
...
- [ ] **TLS Support**:
  ...
  - **Dependencies**: Depends on **Refine Socket Constraints**, `:socket`, `:errors`.
...
- [ ] **Remove OptionHandlerRegistry Operator**:
  ...
  - **Steps**:
    - Analyze usage of `operator[]` in `:protocol_fsm`, `:socket`, and other components.
  ...
  - **Dependencies**: Affects `:internal`, `:protocol_fsm`, `:options`, `:socket`.
...
- [ ] **Refactor `InputProcessor` Helper Methods (Main Loop)**:
  - **Task**: Refactor helper methods to simplify the main processing loop in `InputProcessor::operator()`.
  - **Steps**:
    - Identify reusable logic blocks (e.g., loop setup, byte consumption) in the `PROCESSING` state.
    - Extract and move this logic into private helper methods within `InputProcessor`.
    - Simplify the `PROCESSING` state's `switch` case for better readability.
  - **Dependencies**: Affects `:socket` (`InputProcessor`).
  - **Priority**: Medium (code clarity and maintainability).
  - **Estimated Effort**: 1 day (0.5 for extraction, 0.5 for testing).

- [ ] **Refactor `InputProcessor` Helper Methods (`std::visit`)**:
  - **Task**: Refactor overloaded helper methods for `std::visit` within the `InputProcessor::operator()` response handling.
  - **Steps**:
    - Create a standalone overload set (or a dedicated class/struct) for handling the `ProcessingReturnVariant`.
    - Move the `std::visit` lambda's logic into this separate, named entity.
    - Replace the lambda in `InputProcessor` with a call to `std::visit` using the new helper.
  - **Dependencies**: Affects `:socket` (`InputProcessor`).
  - **Priority**: Medium (code clarity and maintainability).
  - **Estimated Effort**: 1–2 days (0.5 for refactoring, 0.5–1 for testing).

- [ ] **Implement Generic `spawn_handler`**:
  ...
  - **Steps**:
    - Define a templated `spawn_handler(Self&& self, TaggedAwaitable<Tag, T, Awaitable>&& task)` function that:
      ...
      - Optionally integrates with logging or tracing facilities for debug visibility.
    ...
    - Add overloads or tag-based specializations if certain handler types need pre/post-processing (e.g., `OptionTag` may require negotiation state updates before `co_await`).
    - Validate correct propagation of completion signals and error codes to `InputProcessor::complete()`.
  - **Dependencies**: Depends on **Implement Tagged Awaitables**; affects `:socket` (`InputProcessor`), `:awaitables`, `:protocol_fsm`, `:internal`.
  - **Priority**: Medium (code reuse, consistency, and future extensibility).
  - **Estimated Effort**: 1.5–2 days (0.5 for design, 1 for refactoring/integration, 0.5 for testing).
...
- [ ] **Error Conditions**:
  ...
  - **Dependencies**: Extends `:socket` error category; affects all Telnet error propagation, async read loops, and handler signaling.
...
- [ ] **Optimize Synchronous I/O Performance**:
  ...
  - **Priority**: Low (deferred until after Phase 6).
  - **Estimated Effort**: 2–3 days (1 for evaluation, 1–1.5 for implementation, 0.5 for testing).
...
- [ ] **Enhanced Logging with Levels**:
  ...
  - **Dependencies**: Affects `:protocol_fsm`, `:socket`, `:errors`.
  - **Priority**: Medium.
...
- [✔️] **Phase 5: Review Options List** (Completed October 2025):
  ...
  - **Steps**:
    ...
    - Tested integration with `:protocol_fsm` (implemented in `telnet-protocol_fsm-impl.cpp`), `:socket` (implemented in `telnet-socket-impl.cpp`, `telnet-socket-async-impl.cpp`, `telnet-socket-sync-impl.cpp`), and `:internal`.
...
- [✔️] **Phase 5: Review Option Implementation Strategy** (Completed October 2025):
  ...
  - **Steps**:
    ...
    - Tested with `:socket` negotiation methods (implemented in `telnet-socket-impl.cpp`, `telnet-socket-async-impl.cpp`, `telnet-socket-sync-impl.cpp`) and `:errors` (e.g., `user_handler_forbidden`).
  - **Dependencies**: Affects `:options`, `:protocol_fsm`, `:socket`, `:internal`, `:errors`.
  - **Priority**: High (defines option handling architecture).
  - **Estimated Effort**: Completed in 2 days.

- [✔️] **Phase 5: Restructure `socket` with Side Buffers** (Completed October 2025):
  - **Task**: Add input and output side buffers to `socket` for internal buffering of reads and writes.
  - **Steps**:
    - Added `asio::streambuf` members to `socket` providing input and output side buffers.
...
- [✔️] **Phase 5: Implement "Soft EOF" Error Codes for Completing Reads Early** (Completed October 2025):
  ...
  - **Dependencies**: Affects `:errors`, `:protocol_fsm`, and `:socket`.
...
- [✔️] **Phase 5: Refactor Soft EOF Codes** (Completed October 2025):
  ...
  - **Dependencies**: Affects `:errors`, `:protocol_fsm`, `:socket`.
  - **Priority**: High (enhances processing signals for more commands; necessary for Synch/DM correctness).
  - **Estimated Effort**: Completed in 1 day.

- [✔️] **Phase 5: Centralize Command Processing in FSM** (Completed October 2025):
  ...
  - **Steps**:
    ...
    - The calling layer (`telnet::socket::InputProcessor`) was made responsible for managing side buffers, urgent processing state, and notifying user code based on the returned error code.
  ...
  - **Dependencies**: Depends on **Refactor Soft EOF Codes**. Affects `:protocol_fsm` and `:socket` (`InputProcessor`).
  - **Priority**: High (architecture cleanup, aligns with **Eliminate User Command Handlers**).
  - **Estimated Effort**: Completed in 2 days.

- [✔️] **Phase 5: Eliminate User Command Handlers** (Completed October 2025):
...
- [✔️] **Phase 5: Urgent Data/Synch Procedure Handling (OOB)** (Completed October 2025):
  - **Task**: Implement Telnet Synch procedure handling using Out-of-Band (OOB) data for urgent processing.
  - **Steps**:
    - Set `out_of_band_inline` on `lowest_layer()` in `socket` constructor (`telnet-socket-impl.cpp`) using non-throwing `set_option` with `std::error_code&`, logging errors via `ProtocolConfig::log_error` (RFC 854).
    - Added `urgent_data_tracker` class in `telnet-socket.cppm` with `UrgentDataState` enum (`NO_URGENT_DATA`, `HAS_URGENT_DATA`, `UNEXPECTED_DATA_MARK`) and `std::atomic<UrgentDataState>` to track Synch mode, replacing `std::atomic<bool> urgent_flag_`.
    - Added `launch_wait_for_urgent_data` in `socket` to call `async_receive` with `message_out_of_band` on `lowest_layer()` using a 0-byte buffer, guarded by `!context_.urgent_data_state`.
    - Added `async_send_synch` and both `send_synch` overloads in `socket` to send Telnet Synch (three NUL bytes, one as OOB, followed by IAC DM).
    ...
    - Updated `TelnetSocketConcept` in `telnet-socket.cppm` to include `async_receive`, `async_send`, and `set_option` for OOB support.
  - **Dependencies**: Affects `:socket` (`InputProcessor`), `:protocol_fsm` (for DM handling), `:errors`. Depends on **Refactor Soft EOF Codes**.
  - **Priority**: High (critical Telnet feature for interrupt/abort).
  - **Estimated Effort**: Completed in 3 days.

- [✔️] **Phase 5: Implement Tagged Awaitables** (Completed October 2025):
  ...
  - **Steps**:
    ...
    - Tested with `:socket` for subnegotiation handling and `:errors` for error cases (e.g., `user_handler_not_found`).
  - **Dependencies**: Affects `:awaitables`, `:socket`, `:protocol_fsm`, `:internal`.
  - **Priority**: Medium (code clarity, maintainability, and extensibility).
  - **Estimated Effort**: Completed in 1 day.

- [✔️] **Phase 5: Enhance `OptionHandlerRecord`** (Completed October 2025):
  ...
  - **Steps**:
    ...
    - Added `register_option_handlers` and `unregister_option_handlers` in `telnet::socket` (`telnet-socket.cppm`), supporting `option::id_num` with implicit conversion from `option`.
    - Tested with negotiation scenarios (e.g., `WILL ECHO`, `DO NAWS`) using `:socket` and `:errors` (e.g., `invalid_negotiation`, `user_handler_not_found`).
  - **Dependencies**: Affects `:internal`, `:protocol_fsm`, `:options`, `:socket`, `:errors`, `:awaitables`.
  - **Priority**: High (enhances option state management).
  - **Estimated Effort**: Completed in 1 day.

- [✔️] **Phase 5: Invoke Option Negotiation Handlers** (Completed October 2025):
  ...
  - **Steps**:
    ...
    - Updated `telnet::socket::InputProcessor` in `telnet-socket-impl.cpp` to handle `OptionEnablementAwaitable` and `OptionDisablementAwaitable` via `std::visit` like we do for `SubnegotiationHandler`.
  - **Dependencies**: Depends on `:internal`, `:options`, `:protocol_fsm`, `:socket`, `:errors`, `:awaitables`.
  - **Priority**: High (enhances flexibility for option-specific behavior).
  - **Estimated Effort**: Completed in 1 day.

- [✔️] **Phase 5: Restructure Option Negotiation for Internal Implementation** (Completed October 2025):
  ...
  - **Dependencies**: Depends on `:options`, `:internal`, `:protocol_fsm`, `:socket`, `:errors`.
  - **Priority**: High (improves performance for core options).
  - **Estimated Effort**: 0 days (implicitly completed by previous efforts).

- [✔️] **Phase 5: Option Negotiation Method** (Completed October 2025):
  ...
  - **Steps**:
    ...
    - Added `async_request_option` to `telnet::socket` in `telnet-socket.cppm` and send `IAC` `WILL`/`DO` using `async_write_negotiation`.
    - Added a complementary `async_disable_option` to `telnet::socket` method for user code to disable an option and send `IAC` `WONT`/`DONT` via `async_write_negotiation`.
...
- [✔️] **Phase 5: Implement Status Option Internally** (Completed October 2025):
...
- [✔️] **Phase 5: Option Registry Optimization** (Completed October 2025):
...
- [✔️] **Phase 5: Async Negotiation Awaiting** (Completed October 18, 2025):
  ...
  - **Dependencies**: Depends on **Option Negotiation Method**, `:protocol_fsm`, `:socket`, `:types`, `:options`, `:errors`, `:internal`, `:awaitables`.
  - **Priority**: Medium (enhances usability).
  - **Estimated Effort**: Completed in 0.5 days.

### Notes
- Version 0.5.0 reflects the completion of Phase 5, including tagged awaitables, enhanced `OptionHandlerRecord`, and user-initiated option negotiation.
...
- Next focus: Phase 6, **Planning Phase 6 Tasks**
