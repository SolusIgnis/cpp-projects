...
export namespace base::vocab::inline ptr {
     ...
     *
     * @see `required_ptr` for non-null aliasing, `dependency_ptr` for dependency injection structural non-nullability, `cursor_ptr` for non-null arithmetic traversal, `iterator_ptr` for nullable arithmetic traversal, `std::unique_ptr` and `std::shared_ptr` for ownership.
     */
    ...
    /**
     * @brief Deduction guide for `alias_ptr` from references.
     *
    ...
    alias_ptr(T&&) -> alias_ptr<std::remove_reference_t<T>>;

    /**
     * @brief Deduction guide for `alias_ptr` from pointers.
     *
     * @tparam P A pointer-like type with an `element_type` exposed through `std::pointer_traits`.
     */
    template<PointerWithElementType P>
    alias_ptr(P) -> alias_ptr<typename std::pointer_traits<P>::element_type>;

    /**
     * @brief Deduction guide for `alias_ptr` from address-resolvable types.
     *
    ...
    alias_ptr(P) -> alias_ptr<address_resolved_element_t<P>>;
