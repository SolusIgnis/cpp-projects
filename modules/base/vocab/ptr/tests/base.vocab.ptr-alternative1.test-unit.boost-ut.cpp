...
int main()
{
    ...
    "const element forbids mutation through dereference"_test = [] mutable {
        test_each_pointer_type_with([]<template<typename> typename ConcretePtr> {
            ...
            expect(that % !std::is_assignable_v<decltype(*ptr), std::int32_t>);
        });
    };
    ...
    "const pointer prevents rebinding but not mutation"_test = [] mutable {
        test_each_pointer_type_with([]<template<typename> typename ConcretePtr> {
            ...
            *ptr = expected;
            ...
            //Compile-time: ptr must NOT be assignable
            expect(that % !std::is_assignable_v<decltype(ptr)&, const decltype(ptr)&>);
    ...
    "`address_type` nested type preserves top-level const"_test = [] mutable {
        test_each_pointer_type_with([]<template<typename> typename ConcretePtr> {
            ...
            expect(that % std::same_as<typename t::address_type, const std::int32_t*>);
        });
    };
    ...
    "`reference` nested type preserves const"_test = [] mutable {
        test_each_pointer_type_with([]<template<typename> typename ConcretePtr> {
            ...
            expect(that % std::same_as<typename t::reference, const std::int32_t&>);
        });
    };
...
#pragma GCC diagnostic pop
    ...
    //constexpr and std lib customization points
