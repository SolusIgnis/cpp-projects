...
int main()
{
    ...
    "vocabulary pointers interoperate with unordered associative containers"_test = [] mutable {
        ...
        expect(that % test_set.contains(lookup));
        ...
        expect(that % test_set.contains(nullptr));
    ...
    "vocabulary pointers observe but never participate in ownership"_test = [] mutable {
...

        {
            ...
            expect(eq(shared.use_count(), original_count)) << "use count unchanged after alias construction";
            alias->extra = expected;
        } //~alias
        expect(eq(shared.use_count(), original_count)) << "use count unchanged after alias destruction";
