 ...
 * @remark Defines concept to constrain `protocol_fsm` configurations.
...
export namespace net::telnet::concepts {
     ...
     * @brief Constraint on configuration types for `protocol_fsm`.
     * @tparam T Configuration type
     * @remark Ensures `T` provides required types and operations for `protocol_fsm` initialization and behavior.
     * @see `:protocol_fsm` for `protocol_fsm`, `:protocol_config` for `default_protocol_fsm_config`, RFC 854, RFC 855, RFC 1143
     */
    template<typename T>
    concept protocol_fsm_config =
        requires(T& config, telnet::command cmd, option full_opt, option::id_num opt, std::error_code& ec_out, std::error_code ec, byte_t byte, std::string msg) {
            typename T::unknown_option_handler_type;
            requires std::convertible_to<typename T::unknown_option_handler_type, typename protocol_fsm<T>::unknown_option_handler_type>;
